            ...
            <div class="panel panel-success">
                <div class="panel-heading">
                    <div style="font-size: 20px">
                        <div align="center"><h4 style="font-size:40px; color: #ff5a19;">SN编码规则设定</h4></div>
                        ...
                        <div id="form">
                            <table class="table table-hover" style="width: 80%">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        规则名称：<input type="text" id="snrulename">
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                        码长度：<input type="text" id="snlength">
                                    </td>
                                    <td>
                                        码类型：
                                    ...
                                    <td>
                                        <input type="button" value="保存" class="btn btn-default" onclick="saveSNlabel()">
                                    </td>
                                </tr>
                                </thead>
                ...
                <div style="float: left;width: 40%;height: 500px;overflow-x: auto;overflow-y: auto; background-color:  #9cc0ff" id="tab" >
                    <table class="table table-bordered table-hover" id="myTable02" >
                        ...
                        <tbody id="table">
                        <c:forEach items="${snrules}" var="snrules" varStatus="obj">
                            <tr>
                                <td>${obj.count}</td>
                                <td>${snrules.snrulename}</td>
                                <td>${snrules.snlength}</td>
                                <td>${snrules.erp}</td>
                            </tr>
                        </c:forEach>
                        </tbody>
                ...
                <script type="text/javascript">
                    ...
                    function addhang() {
                        ...
                        tr.appendChild(createNode("td", "<select id='snrule"+id+"'  onchange='snrule()' >" +
                                                            "<option >——请选择——</option>" +
                    ...
                    function snrule() {
                        ...
                        var i=1;
                        for (i==1;i<id;i++){
                            var r=document.getElementById("snrule"+i).value;
                            //alert(i+";"+id)
                            switch (r) {
                                case "年":
                                    $("#year"+i).show();
                    ...
                    //保存SN规则数据
                    function saveSNlabel() {

                        if (id==1){
                            alert("请添加规则！");
                        }else {
                            var snrulename=document.getElementById("snrulename").value;
                            var snlength=parseInt(document.getElementById("snlength").value);
                            ...
                            for (var i=1;i<id;i++){
                                ...
                                var rule=document.getElementById("snrule"+i).value;
                                var mesg;
                                switch (rule) {
                                    case "年":
                                ...
                                $.ajax({
                                    url:"/label/insertsnrule",
                                    data:{"snrulename":snrulename, "snlength":snlength, "erp":erp, "start":start, "over":over, "rule":rule, "mesg":mesg},
                                    type:"POST",
                                })
                            }
                            alert("保存成功！");
                            window.location.href="/label/selectsnrule";
                        }
                            ...
                            success:function (res) {
                                ...
                                if (res.code==100){
                                    ...
                                    for (var i=0;i<rules.length;i++)
                                    {
                                        ...
                                        tr.appendChild(createNode("td",rules[i].start));
                                        tr.appendChild(createNode("td",rules[i].over));
                                        tr.appendChild(createNode("td", rules[i].rule));
                                        tr.appendChild(createNode("td", rules[i].mesg));
