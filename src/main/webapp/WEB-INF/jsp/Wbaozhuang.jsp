        ...
        function addsql() {
            ...
            for (var i=1;i<rows.length;i++)
            {
                ...
                if(rows[i].cells[3].innerHTML=="无")
                {
                    ...
                    //alert(mynbox[i]);
                    $.ajax({
                        url: "/pack/updatewbox/" + mynbox[i] + "/" + Wboxid,
                        type: "PUT",
                        data: {"operator": "<%=session.getAttribute("name")%>"},
                    });
                    a+=mynbox[i]+"/";
                    //j++;
                }
            ...
            //打印外箱
            var selectLine= $("#selectLine option:selected").val();
            //   alert(selectLine);
            //打印外箱
            $.ajax({
                url:a+"/",
                type:"GET",
                data:{"selectLine":selectLine}
            });
            print();
            //清空表格
        ...
        function  print() {
            ...
            $.ajax({
                ...
                data:{"Product_Name":Product_Name,"Lot_No":Lot_No,"P_N":P_N,"SPEC":SPEC,"Model_No":Model_No,"QTY":QTY,"Date":Date,
                    "MEAS":MEAS, "GW":GW,"NW":NW,"selectLine":selectLine},
